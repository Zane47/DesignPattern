...
现在呢我们来学习策略是首先呢好多年的策略故事的。也行了算法家族分别的封装起来，让他们之间可以相互替换，此模式呢让算法的变化不会影响到使用算法的用户简单理解使用算法用户就是应用层帮我比如说一会儿我们写的这么一个客户端测试来说，也就是我们的算法封装的不同的类，那么年轻。然后呢让他们直接给相互，那策略模式啊，实际应用有很多，比如说田鸡赛马，你们怎么去赛马这些呢？就是策略我说他那还有促销满减，理解他很少，因为他等等这些呢也都是促销的策略，那扩展呢我们看一下。实际的应用可以大范围的处理掉Else也就是说如果每个方法里面有大量的if else者呢，然后在这样的代码呢？可以通过策略模式给他们消除掉，那它的类型呢是行为型，我把我们看一下策略模式的是有场景，自己自己不太系统，有很多类。而他们的区别呢仅仅在于他们的行为不同，那我们使用策略模式呢就可以动态的让一个对象在许多行为中选择一种行为，也就是说我们把这个对象不同的行为放到那里边，它有很多行为，而每一个行为呢对应每一种策略。一个系统需要动态的在几种算法中选择一种，那这里边所说的算法就是策略，因为策略我的工装就是一系列逻辑的计算方式。比如说拿到两个数字，那通过加法策略就满我们可以我们可以不修改原有系统的基础上的选的选具体的行为，当然了，这种行为我们也可以扩展。这里所说的行为也就是刚刚说的加减乘除这么一，然后呢可以避免使用多种条件转移语句，比如说大量的还有呢一位等等条件专业均我们把具体的策略行为分离成一个一个的单独的类。来替换原来的if else里面的，这样写的好处也会降低代码，然后还有难度，提高算法的保密性和安全性。因为它有我们在使用，我只知道这个策略是干嘛的就可以了，我不需要了解这个策略里边的内部是打个比方，如果我们封装了一个促销的服务，对外提供的呢是不同的策略。客户端在使用的时候只需要使用double的接口，假设呢我们使用的是double，而具体的策略实现的是放到这个double的上。刚刚那是打个比方，那再进一步来说，在具体的策略类中，我们封装了不同的行为和算法以及的相关的数据结构，这个好像对于客户端来说这个是那个是不需要客端知道因为是多么或者使用什么数据结构的，ok，那我们再看一下策略模式有哪些缺点呢？可是这个客户端我们必须知道所有的策略类。并且呢自行来决定使用哪一个。Ok还有呢会产生很多特别累，打个比方，原来呢我们把不同的策略封装到一个方法里面，里面呢通过大量的来判断这些呢，我们都写在一个类里面。如果我们拆出来一个类，一个行为，一个类一个策略，那我就会产生很多的策略实现内容，ok，这个呢很容易理解，那我们再看一下。策略模式相关的设，首先呢看一下策略模式和工厂模式，那这个工厂模式啊就包含扩展方法以及抽象工厂，那工厂模式呢是创建型的设计模式，策略模式呢是行为型的。也就是说工厂模式接制品和创建出符合要求的具体对象，而策略模式呢？他接受已经创建好的对象，从而呢实现不同的行为，还有呢策略模式和状态模式，那状态模式呢包括后面看到你回来见，那这俩呢？什么相关性与区别呢？那在使用策略模式的时候，我们的客户端也就是说需要知道你们到底选择哪一个策略。而我们在使用状态模式，你妈妈是不是客户端是不需要关心体的状态的？那这些状态呢会自动转换，那这两个设计模式呢总结来看，如果系统中某个类的对象存在多种态，那在不同状态下的行为又有差异的话，比如说英雄联盟里边英雄在有buff，还没有爸爸的时候啊，可能会产生不同的行为。当我有爸爸的时呢会比较浪，如果没有buff呢就不会太浪，刚刚举的这个例子啊，比较主观，那也有打的保守的，即使呢我有buff也不浪，那么言归正传啊，那在不同状态下，如果他们的行为有差异的话，而且呢这些状态可以生转换时，我们可以使用状态模式，那如果系统中某个类的为行为你在多种实现方式和后面的我定的时候也会举个具体的业务场景，比如说布克瓦618和双十一的促销这边的策略，那对于促销还是一个行为，而这一行为呢有多种实现方式，这种情况下我们就要使用策略模式，ok，嗯。那接下来，办到了我们能过一起来领，同时呢讲解一下ul，然后呢对策略模式在源码中的一些应用进行解析，ok啊，接下来呢需要办吧，我们这我们一起来扣零吧，9点到我要从虹桥从虹坐地铁到公司，然后去公司数据找的地方都不认识。不然的话我进不去公司。可以说，哎呀，不要这么说。
