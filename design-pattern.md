...
public class Test {
    public static void main(String[] args) {
        ICourse iCourse = new JavaCourse(96, "Java从零到企业级电商开发", 348d);
...
public class Test {
    public static void main(String[] args) {
        ...
        System.out.println("课程ID:" + javaCourse.getId() + " 课程名称:" + javaCourse.getName() + " 课程价格:" + javaCourse.getDiscountPrice() + "元");
    }
}
```

如果课程实现类很多, 那么就都需要这么改, 实现这个方法.

接口不应该经常变化, 应该是稳定且可靠的, 类似于契约作用.

面向接口编程, 



新建JavaDiscountCourse类继承JavaCourse, 并且重写父类的getPrice方法
...
* 优点
  * 在内存里只有一个实例，减少了内存开销. 特别是对象需要频繁地创建和销毁
  * 可以避免对资源的多重占用
  * 设置全局访问点，严格控制访问. 对外不可以直接new, 需要通过我的方法
* 缺点
  * 没有接口, 扩展困难

#### 重点

* 私有构造器(禁止从单例类外部使用构造函数创建对象)
* 线程安全(重要!)
* 延迟加载. 使用时再创建
* 序列化和反序列化安全. 
* 反射. 防止反射攻击
...
<img src="asset/img/design-pattern/image-20221130225650910.png" alt="image-20221130225650910" style="zoom:50%;" />

* 静态内部类实现方案--基于类初始化的延迟加载解决方案

<img src="asset/img/design-pattern/image-20221130225702964.png" alt="image-20221130225702964" style="zoom:50%;" />

#### ps, 实用技能

反编译, 内存原理, 多线程debug(分布式锁调试用到了多进程debug)

#### 相关设计模式

单例模式和工厂模式, 工厂类设置为单例

单例模式和享元模式, 业务场景中需要管理很多业务对象, 结合完成单例的获取, 这里享元就类似工厂, 只是拿出已经创建好而不是直接创建的对象
